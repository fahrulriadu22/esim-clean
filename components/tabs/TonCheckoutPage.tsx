...
import { useTonConnect } from "@/lib/ton-connect";
import { getTonPrice, convertUsdToTon } from "@/lib/coingecko";
import { Package } from "@/hooks/usePackages";
import { RECIPIENT_ADDRESS, formatTonAddress } from "@/lib/ton-connect";
import { Address, toNano } from "@ton/core";

interface TonCheckoutPageProps {
   plan: Package;
   onBack: () => void;
   onSuccess: () => void;
}
...
export function TonCheckoutPage({
   ...
   const loadTonPriceAndInitPayment = async () => {
      ...
      try {
         ...
         const tonAmount = await convertUsdToTon(plan.price);
         ...
         try {
            const response = await fetch(
               `/api/verify-ton?senderAddress=${encodeURIComponent(
                  senderAddress
               )}&sku=${encodeURIComponent(plan.code)}`,
               {
                  method: "GET",
                  headers: {
                     Accept: "application/json",
